...
def _zeroclaw_loop():
    ...
    while _zeroclaw_running.is_set():
        try:
            ...
            if hands_dir.exists():
                for _ in range(10):  # Process a few hands per iteration
...
def _build_dashboard(start=None, end=None, run_id=None) -> dict:
    ...
    summary["zeroclaw"] = {
        ...
        "summary": "ZeroClaw agents active" if _zeroclaw_running.is_set() else "ZeroClaw not running",
    }
...
def health():
    return {
        ...
        "zeroclaw": _zeroclaw_running.is_set(),
    }
...
def agents_live():
    ...
    hands.append({
        ...
        "findings": [f"Attack simulation {'active' if _sim_running.is_set() else 'idle'}", f"Interval: {ATTACK_INTERVAL}s"],
        "steps": [
        ...
        "metrics": {"simulation_interval": ATTACK_INTERVAL, "engines": 3},
        "duration_ms": 0,
    })
    hands.append({
        ...
        "findings": [f"ZeroClaw daemon {'active' if _zeroclaw_running.is_set() else 'idle'}"],
        "steps": [{"title": "Daemon", "detail": "Running" if _zeroclaw_running.is_set() else "Stopped", "status": "active" if _zeroclaw_running.is_set() else "idle"}],
        "metrics": {"daemon": "active" if _zeroclaw_running.is_set() else "idle"},
        "duration_ms": 0,
    })
...
def zeroclaw_status():
    ...
    return {
        ...
        "daemon": {
            "reachable": False,
            "url": "",
            "status": "native" if _zeroclaw_running.is_set() else "not_configured",
            "paired": False,
...
def zeroclaw_health():
    ...
    if port:
        try:
            ...
            pass
    return {"available": _zeroclaw_running.is_set()}
