...
3. **Detection** — 6000+ rules (Sigma, Elastic, Wazuh, Panther) with BM25 search, plus five streaming capture detectors: port scan (T1046), SSH brute force (T1110), SYN flood (T1498), C2 beaconing (T1071), and data exfiltration (T1041). The **rule engine** (`app_shared/rule_engine.py`) compiles ~6.5k of the indexed rules into live event matchers — sigma 99.8%, elastic 85%, wazuh 84%, panther 85% — and every captured event is evaluated against all of them each detection pass; a match fires an alert citing the rule's id/engine with AI triage + response preview attached. Captured packets carry a 512-byte payload snapshot (TCP+UDP) so content rules (reverse shells, SQLi strings, /etc/shadow reads, webshells) match on wire contents, not just metadata. Skipped rules are counted with reasons (ESQL, threat-match joins, enrich-index-only queries, upstream `return True` stubs) — a matcher that fires on nothing is never loaded.
